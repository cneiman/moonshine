...
Tools:
  - memory_context    — load relevant memories for session start
  - memory_search     — hybrid FTS5 + semantic + graph search
  - memory_save       — persist a new memory with auto-embedding + entity extraction
...
Transport: stdio (for Claude Code, OpenClaw, Cursor, or any MCP client)
...
def tool_memory_context(params: dict) -> str:
    ...
    # Check for today's daily file in workspace
...
def tool_memory_search(params: dict) -> str:
    ...
    # If spread requested, use recall_with_spread
    if spread:
        spread_results = recall_with_spread(
            conn, query, limit=limit, spread=True, max_hops=2, decay=0.5,
        ...
        return f"Found {len(spread_results)} memories (with spread):\n\n" + '\n'.join(lines)
    ...
    for score, row in results:
        ...
        star = '★' * row.get('importance', 3)
        lines.append(f"**#{row['id']}** [{row['type']}] {row['title']}{score_str} {star}")
        if row.get('content'):
            lines.append(f"  {row['content'][:300]}")
...
TOOLS = [
    ...
    {
        "name": "memory_search",
        "description": "Search the long-term memory database. Uses semantic similarity (Ollama embeddings) by default, falls back to keyword search (FTS5). Use this to find specific past decisions, lessons, events, or patterns.",
        "inputSchema": {
            "type": "object",
            "properties": {
                "query": {
                    "type": "string",
                    "description": "Search query — can be natural language for semantic search"
                },
                "semantic": {
                    "type": "boolean",
    ...
    if method == "initialize":
        ...
        try:
            tool_map = {
                ...
                "memory_briefing": tool_memory_briefing,
                "memory_surface": tool_memory_surface,
                "memory_entities": tool_memory_entities,
                "memory_connect": tool_memory_connect,
                "memory_neighbors": tool_memory_neighbors,
